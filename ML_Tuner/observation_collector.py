...
class ObservationCollector:
    ...
    def _collect_latency(self, features: Dict[str, float]):
        """
        ★ 修正: 使用 EventFeature-Sink 的自定义 latency_p99 指标
        而非复杂的 Job 级 LatencyMarker

        已验证可用:
          Sink__EventFeature-Sink.latency_p99 (subtasks/metrics, max = 最差 subtask)
        """
        ...
        metric_name = f"{prefix}.latency_p99"

        data = self._query_subtask_metrics(event_sink_vid, [metric_name])

        latency_data = data.get(metric_name, {})
        # 取 max = 所有 subtask 中最差的 P99
        features['latency_p99'] = latency_data.get('max', 0.0)

        logger.debug("延迟: P99=%.1f ms", features['latency_p99'])
